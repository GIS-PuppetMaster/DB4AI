operator DNN(acc,auc,prec,recall,mse,f1,test_x,test_y,x,y,lr,layer_units,iter_times){
    ...
    create tensor w_0(feature_num,lu_0) from random((feature_num,lu_0),(0,0.5)) with grad
    create tensor b_0(1,lu_0) from zeros((1,lu_0)) with grad
    create tensor w_1(lu_0,lu_1) from random((lu_0,lu_1),(0,0.5)) with grad
    create tensor b_1(1,lu_1) from zeros((1,lu_1)) with grad
    create tensor w_2(lu_1,2) from random((lu_1,2),(0,0.5)) with grad
    create tensor b_2(1,2) from zeros((1,2)) with grad
    ...
    LOOP(iter_times){
        ...
        select -MEAN(y*LOG(output_2)+(1-y)*LOG(1-output_2)) as loss
        # select MEAN(POW(y-output_2,2)) as loss

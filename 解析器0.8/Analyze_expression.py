...
def Analyze_expression(result, x):
    ...
    if requires_grad is True and len(expression) >= 3:
        if expression[len(expression) - 3] == 'WITH' and expression[len(expression) - 2] == 'GRAD':
            expression.pop(len(expression) - 3)
            expression.pop(len(expression) - 2)
    print(expression)
    for i in expression:
    ...
    for i in expression:
        if i == '(':
            ...
            currentgraph.setVal(nd.InstantiationClass(x + len(G.nodes), 'CreatTensor',
                                                      data_shape='(' + str(math.pi) + ')', with_grad=requires_grad))
            parent = newStack.pop()
            G.InsertNode(currentgraph.keynode)
            ...
            currentgraph.setVal(nd.InstantiationClass(x + len(G.nodes), 'CreatTensor',
                                                      data_shape='(' + str(math.e) + ')'), with_grad=requires_grad)
            parent = newStack.pop()
            G.InsertNode(currentgraph.keynode)
            if currentgraph != parent:
                G.InsertEdge(currentgraph.keynode, parent.keynode)
    ...
    for n in G.nodes:
        ...
        if flag == 0:
            top_node = n
            top_node_list = [top_node, top_node.id]
            break
    return G, vallist, top_node_list


if __name__ == '__main__':
    s = 'DEF A = ( B + C ) * D WITH GRAD'
